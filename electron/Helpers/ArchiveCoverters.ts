...
import {createExtractorFromFile, ExtractorFromFileOptions} from 'node-unrar-js/esm';
...
async function ExtractRAR(inPath: string, tempFolder: string) {
    try {
        ...
        const wasmPath = path.join(baseDir, 'node_modules/node-unrar-js/esm/js/unrar.wasm');
        ...
        console.log(`Extraction complete. ${files.length} entries processed for archive: ${inPath}`);
...
async function ArchiveFile(
    ...
    try {
        if (extension === ".zip")
            ...
            await ExtractRAR(inPath, tempFolder)
        else
...
export async function ToRAR(outDir: string, inPath: string, name: string, extension: string) {
    return await ArchiveFile(outDir, inPath, name, "rar", extension)
}
export async function ToGZ(outDir: string, inPath: string, name: string, extension: string) {
    return await ArchiveFile(outDir, inPath, name, "gz", extension)
}
export async function ToBZ2(outDir: string, inPath: string, name: string, extension: string) {
