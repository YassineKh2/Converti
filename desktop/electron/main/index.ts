...
import {appSettings, defaultSettings} from "../Helpers/getDefaultSettings";
import {
    Make7ZArchive,
    MakeBZ2Archive,
    MakeGZArchive,
...
const appSettingsPath = path.join(appData, "appSettings.json");
...
ipcMain.handle("appSettings", async (_, arg) => {

    let appSettings: InternalAppSettings = arg;
    if (appSettings) {
        return await writeFile(appSettingsPath, JSON.stringify(appSettings));
    }
    ...
    try {
        const appSettingsRaw = await readFile(appSettingsPath, "utf8");
