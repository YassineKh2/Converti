...
import {Cog, Globe, Package, RotateCcw, Settings as SettingsIcon, Trash2, Upload, User, X, Zap,} from "lucide-react";
...
import {FileGroup, UploadedFile as UploadedFileType,} from "@/type/UploadedFile";
import {getFileCategory} from "@/Helpers/getFileCategory";
import {formatFileSize} from "@/Helpers/formatFileSize";
import {getArchiveOptions, getConversionOptions, getDefaultFormat,} from "@/Helpers/getConversionOptions";
...
export default function FileConverter() {
    ...
    useEffect(() => {
        ...
        getAppSettings().then((appSettings: InternalAppSettings) => {
            setShowOnboarding(appSettings.firstLunch)
        });
    }, []);
    ...
    const handleCantDrop = useCallback((e: DragEvent) => {
        e.preventDefault();
        toast.error("Can't add new files while converting everything")
    }, []);
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-purple-50 to-indigo-100 p-4 flex flex-col justify-between">
            {showOnboarding && (
                <OnboardingSlides onComplete={() => setShowOnboarding(false)} onSkip={() => setShowOnboarding(false)}/>
            )}
            <div className="max-w-6xl mx-auto w-full">
                ...
                <Card className="border-2  border-dashed border-gray-300 hover:border-gray-400 transition-colors mb-6">
                    <CardContent className="p-8">
                        <div
                            className={`text-center ${!isConverting && isDragOver && "bg-secondary/10 border-ring"} ${isConverting && "bg-gray-100 cursor-not-allowed"} rounded-lg p-6 transition-colors`}
                            onDragLeave={handleDragLeave}
                            onDragOver={handleDragOver}
                            onDrop={!isConverting ? handleDrop : handleCantDrop}
                        >
                            ...
                            <div className={"flex items-center justify-center gap-4 "}>
                                <Button disabled={isConverting} asChild className="bg-secondary hover:bg-chart-4">
                                    <label className={!isConverting ? "cursor-pointer" : "cursor-not-allowed"} htmlFor="file-upload">
                                        Choose Files
                                        <input
                            ...
                            <Card className="mb-6 bg-gradient-to-br from-slate-50 to-blue-50 border-slate-200 border-2">
                                ...
                                <CardContent>
                                    <div className="space-y-4">
                                        ...
                                        {selectedArchiveFormat && (
                                            <div
                                                className="flex flex-col gap-2 justify-between pt-2 border-t border-purple-200">
